...
export const StatsionarClients = () => {
  ...
  const checkData = () => {
    ...
    if (checkDiagnos(diagnos)) {
      return notify(checkDiagnos(diagnos));
    }
    ...
    try {
      const data = await request(
        ...
        "POST",
        {
          ...
          room: { ...room },
          diagnos: { ...diagnos, clinica: auth.clinica._id },
        },
        {
          Authorization: `Bearer ${auth.token}`,
        }
  ...
  const postRoom = async () => {
    try {
      const data = await request(
        `/api/statsionatclient/client/end`,
        "POST",
        {
          clinica: auth.clinica._id,
          room: { ...postRoomBody },
